...
impl Catalog for SqlCatalog {
    ...
    async fn update_table(self: Arc<Self>, commit: CommitTable) -> Result<Table, IcebergError> {
        ...
        // Only reclaim aged-out metadata files when our compare-and-swap
        // actually landed: if a concurrent writer won the CAS (0 rows updated),
        // the files in our local log may still be referenced by the winner.
        // Best effort — a failed delete only leaves an orphan, never corrupts
        // state.
        if delete_after_commit && update_result.rows_affected() > 0 {
